...
function Printer() {
    this.colors = [
        'blue', 'black', 'cyan', 'green', 'grey',
        'magenta', 'red', 'white', 'yellow',
        'rainbow', 'zebra', 'america'
    ];
    this.bgColors = [
...
Printer.prototype.PRINT = function print(config) {
    ...
    var bgColor = this.bgColors.matched(config.bgColor),
        inputData = config.text || this.defaults.text;
    ...
    // apply color
    if (config.color) {
        inputData = col[this.colors.matched(config.color, this.defaults.color)](inputData);
    }
